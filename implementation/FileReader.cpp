...
/*
    ...
    gameYear   = dr.rotateLeft(gameYear, 8);
    gameMonth  = dr.rotateLeft(gameMonth, 8);
    gameWeek   = dr.rotateLeft(gameWeek, 8);
    awardsLeft = dr.rotateLeft(awardsLeft, 8);
*/
    for(int i = 0; i < 256; i++)
        itemInventoryBuffer[i] = dr.rotateLeft(itemInventoryBuffer[i], 8);
...
void FileReader::fixTwoByteGeneralsArrays(void)
{
    for(int i = 0; i < 171; i++)
    {
        currExpBuffer[i]      = dr.rotateLeft(currExpBuffer[i], 8);
        totalWinBuffer[i]     = dr.rotateLeft(totalWinBuffer[i], 8);
        totalLossBuffer[i]    = dr.rotateLeft(totalLossBuffer[i], 8);
        winsLastWeekBuffer[i] = dr.rotateLeft(winsLastWeekBuffer[i], 8);
        lossLastWeekBuffer[i] = dr.rotateLeft(lossLastWeekBuffer[i], 8);
        newMeritsBuffer[i]    = dr.rotateLeft(newMeritsBuffer[i], 8);
        oldMeritsBuffer[i]    = dr.rotateLeft(oldMeritsBuffer[i], 8);
        hospitalBuff[i]       = dr.rotateLeft(hospitalBuff[i], 8);
        negoBuff[i]           = dr.rotateLeft(negoBuff[i], 8);
        locationNumBuff[i]    = dr.rotateLeft(locationNumBuff[i], 8);
        domesticStatusBuff[i] = dr.rotateLeft(domesticStatusBuff[i], 8);
        defSwordBuff[i]       = dr.rotateLeft(defSwordBuff[i], 8);
        defIceBuff[i]         = dr.rotateLeft(defIceBuff[i], 8);
        defFireBuff[i]        = dr.rotateLeft(defFireBuff[i], 8);
        defLightBuff[i]       = dr.rotateLeft(defLightBuff[i], 8);
        defDarkBuff[i]        = dr.rotateLeft(defDarkBuff[i], 8);
    }
...
void FileReader::fixTwoByteCastlesArrays(void)
{
    for(int i = 0; i < 34; i++)
    {
        castleCurrentTroops[i] = dr.rotateLeft(castleCurrentTroops[i], 8);
        castleExperience[i]    = dr.rotateLeft(castleExperience[i], 8);
    }
...
void FileReader::fixTwoByteDivisionsArrays (void)
{
    for(int i = 0; i < 171; i++)
    {
        divisionXcoordinate[i]   = dr.rotateLeft(divisionXcoordinate[i], 8);
        divisionYcoordinate[i]   = dr.rotateLeft(divisionYcoordinate[i], 8);
        divisionUnknown1Buff[i]  = dr.rotateLeft(divisionUnknown1Buff[i], 8);
        divisionUnknown2Buff[i]  = dr.rotateLeft(divisionUnknown2Buff[i], 8);
        divisionUnknown3Buff[i]  = dr.rotateLeft(divisionUnknown3Buff[i], 8);
        divisionUnknown4Buff[i]  = dr.rotateLeft(divisionUnknown4Buff[i], 8);
        divisionUnknown5Buff[i]  = dr.rotateLeft(divisionUnknown5Buff[i], 8);
        divisionUnknown6Buff[i]  = dr.rotateLeft(divisionUnknown6Buff[i], 8);
        divisionUnknown7Buff[i]  = dr.rotateLeft(divisionUnknown7Buff[i], 8);
        divisionUnknown8Buff[i]  = dr.rotateLeft(divisionUnknown8Buff[i], 8);
        divisionUnknown9Buff[i]  = dr.rotateLeft(divisionUnknown9Buff[i], 8);
        divisionUnknown10Buff[i] = dr.rotateLeft(divisionUnknown10Buff[i], 8);
        divisionUnknown11Buff[i] = dr.rotateLeft(divisionUnknown11Buff[i], 8);
        divisionUnknown12Buff[i] = dr.rotateLeft(divisionUnknown12Buff[i], 8);
    }
