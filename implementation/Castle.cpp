...
int Castle::changeLeader(const int genIndex)
{
    ...
    newLeader = dr.genArr[generals[genIndex]].getListIndex();
...
int Castle::changeRuler(const int genIndex)
{
    ...
    if((leader == dr.getPlayingAs()) && (leader != genIndex)) //if the player's monarch is in this castle
        return 0;                                        //then you can't change the ruler
    else
        if((leader == dr.getPlayingAs()) && (leader == genIndex)) //if trying to change ruler to the player's monarch when
...
{
    int genTroops = dr.genArr[genIndex].troopMedals[dr.genArr[genIndex].getTroopIndex()] * 10;
    ...
    {                 //used after already initialized
        ...
        ruler = officerOwner[dr.genArr[genIndex].getListIndex()];
...
//        dr.fw.writeOneElementToFile(CASTLE_OWNERSHIP, 1, ruler, castleNum);
        ...
        if(!isCastleFull())
        {
            ...
            if(dr.getPlayingAs() == genIndex) //if player monarch is coming here,
...
{
    if(dr.getPlayingAs() == genIndex) //fail, player monarch can't be captive
        return 0;
