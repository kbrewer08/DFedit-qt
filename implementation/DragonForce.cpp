...
void DragonForce::addItemsInv(const int* const items, const int count)
{
    ...
    if(count > 0)
    {
        ...
        dr.insertionSort((int*)itemInv.items, itemInv.numItems);
...
void DragonForce::delItemsInv(const int* const items, const int count)
{
    ...
    dr.insertionSort(itemInv.items, itemInv.numItems);
...
ushort DragonForce::packByte(ushort highNibble, ushort lowNibble)
{
    return lowNibble | (highNibble << 4);
}

ushort DragonForce::rotateLeft(ushort num, const int bits)
{
    if(bigEndian){
        return num;
    }
...
void DragonForce::insertionSort( int* const arr, const int size ) //standard insertion sort
{
    int temp;
